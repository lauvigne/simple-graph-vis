...
def _(mo):
    intro = "\n".join(
        [
            "# Sankey hiérarchique des applications et capacités",
            "",
            "Cette version montre explicitement:",
            "- Entité",
            "- Application",
            "- BC L3",
            "- BC L2",
            "",
            "Si une application n'est reliée qu'à une BC L2, le lien est direct.",
        ]
    )
    mo.md(intro)
    return
...
@app.cell
def _(build_hierarchical_sankey, mo, model, selected_capabilities):
        ...
        sankey_view = mo.md(
            "\n".join(
                [
                    ...
                    "Le graphe reste volontairement filtré pour éviter un rendu trop dense.",
                ]
            )
        )
    else:
        sankey_view = build_hierarchical_sankey(model, capability_codes=selected_codes)
    sankey_view
    return
